...
export default function ServiceAreasPage() {
  ...
  return (
<main className="py-6 mt-4  bg-white">
  {/* Hero */}
  <section className="mx-auto px-6 text-center mb-10">
    <h1 className="text-4xl md:text-5xl font-bold text-primary">Service Areas Across Dubai</h1>
    <p className="mt-4 text-lg max-w-2xl mx-auto text-black">We provide professional pest control services throughout Dubai and surrounding emirates. No matter where you&apos;re located, we&apos;re here to help.</p>
  </section>

  {/* Area List */}
  <section className="max-w-6xl mx-auto px-4 sm:px-6 mb-16">
    <h2 className="text-2xl sm:text-2xl md:text-3xl font-bold mb-6">Areas We Serve</h2>
    ...
    <div className="text-center mt-10 px-2">
      <p className="mb-2 text-md sm:text-base text-black">We service many more locations throughout Dubai and the UAE. Contact us to check if we cover your specific area.</p>
    </div>
  </section>
  ...
  <section className="py-16">
    <div className="max-w-6xl mx-auto px-6">
      <h2 className="text-3xl md:text-4xl font-bold text-center mb-10">Emergency Coverage</h2>
      <ul className="grid sm:grid-cols-2 md:grid-cols-4 gap-6">
        {emergency.map((item, i) => (
          <li key={i} className="card rounded-xl p-6 text-center flex flex-col items-center">
            ...
            <p className="text-sm mt-2 text-black">{item.desc}</p>
          </li>
        ))}
      </ul>
    </div>
