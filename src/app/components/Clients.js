...
<section className="  py-12 sm:py-16 lg:py-20 bg-secondary">
  <div className="text-center px-4 sm:px-6 lg:px-8 mx-auto max-w-7xl">
    <motion.h2
      variants={fadeUp}
      initial="hidden"
      whileInView="visible"
      viewport={{ once: true }}
      custom={0}
      className="text-2xl sm:text-3xl md:text-5xl font-bold text-primary mb-8 sm:mb-10 font-[var(--font-heading)]"
    >
      Trusted by Leading Clients
    </motion.h2>

    <div className="grid grid-cols-2 xs:grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-6 sm:gap-8 items-center justify-items-center">
      {images.map((src, i) => (
        <motion.div
          key={`client-${i}`}
          variants={fadeUp}
          ...
          viewport={{ once: true }}
          custom={i + 1}
          className="relative w-24 h-14 sm:w-28 sm:h-16 md:w-32 md:h-18 lg:w-36 lg:h-20 transition-transform duration-300 ease-in-out hover:scale-105 grayscale hover:grayscale-0"
        >
          <Image
            src={src}
            alt={`Client logo ${i + 1}`}
