...
console.log(data);
  ...
  return (
<motion.section initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.2 }} variants={fadeUp} className=" max-w-[var(--width-custom)] mx-auto py-16 sm:py-20 px-4 sm:px-6 md:px-12 ">
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8 mt-10">
      {data.detail.map((items, i) => (
        <motion.div key={i} variants={fadeUp} custom={i + 2} className="bg-white rounded-xl p-6 shadow-sm border border-gray-100 hover:shadow-md hover:border-accent transition-all duration-200">
          ...
          <ul className="space-y-2 text-black font-[var(--font-primary)]">
          {items.list.map((point, index) => (
